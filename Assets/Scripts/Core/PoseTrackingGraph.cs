...
namespace Mediapipe.Unity.Sample.PoseTracking
{
    public readonly struct PoseTrackingResult
    {
        ...
        public PoseTrackingResult(Detection poseDetection, NormalizedLandmarkList poseLandmarks,
            LandmarkList poseWorldLandmarks, ImageFrame segmentationMask, NormalizedRect roiFromLandmarks)
        {
    ...
    public class PoseTrackingGraph : GraphRunner
    {
        ...
        public async Task<PoseTrackingResult> WaitNextAsync()
        {
            ...
            _ = TryGetValue(results.Item3.packet, out var poseWorldLandmarks,
                (packet) => { return packet.Get(LandmarkList.Parser); });
            _ = TryGetValue(results.Item4.packet, out var segmentationMask, (packet) => { return packet.Get(); });
            _ = TryGetValue(results.Item5.packet, out var roiFromLandmarks,
                (packet) => { return packet.Get(NormalizedRect.Parser); });

            return new PoseTrackingResult(poseDetection, poseLandmarks, poseWorldLandmarks, segmentationMask,
                roiFromLandmarks);
        }
        ...
        protected override void ConfigureCalculatorGraph(CalculatorGraphConfig config)
        {
            ...
            _roiFromLandmarksStream =
                new OutputStream<NormalizedRect>(calculatorGraph, _RoiFromLandmarksStreamName, true);
        ...
        private PacketMap BuildSidePacket(ImageSource imageSource)
        {
            ...
            Debug.Log(
                $"output_rotation = {outputRotation}, output_horizontally_flipped = {outputHorizontallyFlipped}, output_vertically_flipped = {outputVerticallyFlipped}");
