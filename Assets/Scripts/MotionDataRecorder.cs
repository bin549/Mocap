...
public class MotionDataRecorder : MonoBehaviour {
    ...
    private void LateUpdate() {
        ...
        var LeftFootTQ = new TQ(_animator.GetBoneTransform(IK_LeftFootBone).position,
            _animator.GetBoneTransform(IK_LeftFootBone).rotation);
        var RightFootTQ = new TQ(_animator.GetBoneTransform(IK_RightFootBone).position,
            _animator.GetBoneTransform(IK_RightFootBone).rotation);
        LeftFootTQ = AvatarUtility.GetIKGoalTQ(_animator.avatar, _animator.humanScale, AvatarIKGoal.LeftFoot, bodyTQ,
            LeftFootTQ);
        RightFootTQ = AvatarUtility.GetIKGoalTQ(_animator.avatar, _animator.humanScale, AvatarIKGoal.RightFoot, bodyTQ,
            RightFootTQ);
    ...
    private static void SetHumanBoneTransformToHumanoidPoses(Animator animator,
        ref HumanoidPoses.SerializeHumanoidPose pose) {
...
*/
    ...
    */
    protected virtual void WriteAnimationFile() {
        ...
        Poses.SaveAnim();
        #else
        Debug.LogWarning("SaveAnim 仅在 Unity Editor 可用。");
        #endif
    }
    ...
    public class AvatarUtility {
        static public TQ GetIKGoalTQ(UnityEngine.Avatar avatar, float humanScale, AvatarIKGoal avatarIKGoal,
            TQ animatorBodyPositionRotation, TQ skeletonTQ) {
            ...
            MethodInfo methodGetAxisLength =
                typeof(UnityEngine.Avatar).GetMethod("GetAxisLength", BindingFlags.Instance | BindingFlags.NonPublic);
            if (methodGetAxisLength == null)
                throw new InvalidOperationException("Cannot find GetAxisLength method.");
            MethodInfo methodGetPostRotation =
                typeof(UnityEngine.Avatar).GetMethod("GetPostRotation", BindingFlags.Instance | BindingFlags.NonPublic);
